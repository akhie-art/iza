...
try:
    ...
    st.warning("⚠️ cv2 atau mediapipe tidak ditemukan. Menggunakan mode simulasi.")
    import types
    HAS_CV2 = False
    HAS_MEDIAPIPE = False
    # Dummy cv2 & mediapipe agar kode tidak error
...
# ========== FUNGSI SUARA ==========
def speak(text):
    """Buat suara dari teks menggunakan gTTS"""
    try:
...
def detect_wave(frame):
    if not HAS_CV2 or not HAS_MEDIAPIPE or hands is None:
        # Mode simulasi
        cv2.putText(frame, "Simulasi: Lambaian Terdeteksi", (10, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 0), 3)
        speak("Halo, saya Iza Cantik dalam mode simulasi!")
        return frame, True
...
def main():
    ...
    st.info("Unggah gambar tangan Anda atau gunakan kamera Streamlit. Jika lambaian terdeteksi, Iza akan menyapa Anda!")

    st.header("📤 Unggah Gambar Tangan")
    ...
    if uploaded_file is not None:
        ...
        st.image(processed_frame_rgb, channels="RGB", caption="Hasil Deteksi")
        if wave_detected_now and st.session_state.audio_data:
            st.audio(st.session_state.audio_data, format='audio/mp3')
            st.session_state.audio_data = None

    st.markdown("---")
    st.header("📸 Ambil Foto Langsung")
    ...
    if camera_image is not None:
        ...
        processed_frame, wave_detected_now = detect_wave(frame)
        processed_frame_rgb = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB)
        st.image(processed_frame_rgb, channels="RGB", caption="Hasil Deteksi Kamera")
        if wave_detected_now and st.session_state.audio_data:
            st.audio(st.session_state.audio_data, format='audio/mp3')
            st.session_state.audio_data = None
